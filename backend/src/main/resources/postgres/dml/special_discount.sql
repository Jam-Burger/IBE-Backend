...
-- Clear existing data
TRUNCATE TABLE special_discount;
...
holiday_discounts AS (
    ...
    FROM holidays h
    CROSS JOIN (
        SELECT generate_series(1, 24) as property_id
    ) p
    WHERE RANDOM() < 0.4
),
...
weekend_dates AS (
    SELECT 
        date_series::date as discount_date
...
weekend_discounts AS (
    SELECT 
        (SELECT COALESCE(MAX(id), 0) FROM holiday_discounts) + ROW_NUMBER() OVER () as id,
        p.property_id,
        w.discount_date,
    ...
    WHERE 
        RANDOM() < 0.2
        AND NOT EXISTS (
            SELECT 1 FROM holiday_discounts hd 
...
weekday_discounts AS (
    SELECT 
        (SELECT COALESCE(MAX(id), 0) FROM weekend_discounts) + ROW_NUMBER() OVER () as id,
        p.property_id,
        w.discount_date,
    ...
    WHERE 
        ...
        AND NOT EXISTS (
            SELECT 1 FROM weekend_discounts wd 
            WHERE wd.discount_date = w.discount_date 
            AND wd.property_id = p.property_id
        )
)
-- Combine all discounts and insert into table
INSERT INTO special_discount (id, property_id, discount_date, discount_percentage, description)
...
FROM (
    ...
    UNION ALL
    SELECT * FROM weekday_discounts
) combined_discounts;

-- Display discount statistics by type (for verification)
WITH discount_summary AS (
    SELECT 
        description as discount_type,
        ...
        COUNT(DISTINCT discount_date) as unique_dates
    FROM special_discount
    GROUP BY description
)
SELECT * FROM discount_summary; 